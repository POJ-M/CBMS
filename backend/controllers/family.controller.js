...
const createFamily = catchAsync(async (req, res, next) => {
  ...
  if (!TN_DISTRICTS.includes(district)) return next(new AppError('Invalid district.', 400));
  ...
  // NEW: Check if Deceased status
  const isDeceased = head.membershipStatus === 'Deceased';
  
  // DOB: Required for non-Deceased, optional for Deceased
  if (!isDeceased && !head?.dob) {
    return next(new AppError('Head date of birth is required.', 400));
  }
  
  // Only validate these if NOT Deceased
  if (!isDeceased) {
  ...
  // Calculate age only if DOB provided
  const age = head.dob ? calcAgeIST(head.dob) : null;

  // Age < 18: force Single (only if DOB exists)
  let maritalStatus = isDeceased 
    ? undefined 
    : (age !== null && age < 18 ? 'Single' : head.maritalStatus);
  ...
  try {
    ...
    const headData = {
      ...
      maritalStatus: isDeceased ? undefined : maritalStatus,
      memberType: isDeceased ? 'Deceased' : (head.memberType || 'Member'),
      membershipStatus: head.membershipStatus || 'Active',
      joinDate: head.joinDate || undefined,
      baptized: isDeceased ? undefined : head.baptized,
      baptizedDate: (head.baptized === 'Yes' && head.baptizedDate) ? head.baptizedDate : null,
      weddingDate,
      spouseName,
      occupationCategory: isDeceased 
        ? 'Deceased' 
        : (age !== null && age <= 5 ? 'Child' : head.occupationCategory),
    };
    ...
    res.status(201).json({ 
      ...
      data: { family, head: headBeliever } 
    });
  } catch (err) {
    await session.abortTransaction();
    
    if (err.code === 11000 && err.keyPattern?.familyCode) {
      return next(new AppError(
        'Unable to generate family code. This may be due to deleted records. Please try again.',
        500
      ));
    }
...
const addMember = catchAsync(async (req, res, next) => {
  ...
  // NEW: Check if Deceased status
  const isDeceased = data.membershipStatus === 'Deceased';
  
  // DOB: Required for non-Deceased, optional for Deceased
  if (!isDeceased && !data.dob) {
  ...
  // Calculate age only if DOB provided
  const age = data.dob ? calcAgeIST(data.dob) : null;
  const isSpouseRelation = ['Wife', 'Husband'].includes(data.relationshipToHead);

  // Age check only if DOB exists
  if (age !== null && age < 18) {
    if (isSpouseRelation) {
  ...
  // Auto-set for Deceased
  if (isDeceased) {
    data.memberType = 'Deceased';
    data.occupationCategory = 'Deceased';
  }

  // Spouse logic (only for non-Deceased)
  let autoLinkedSpouse = false;
  if (!isDeceased && isSpouseRelation && (age === null || age >= 18)) {
  ...
  } else if (!isDeceased && !isSpouseRelation && (age === null || age >= 18)) {
  ...
  const memberData = {
    ...
    memberType: isDeceased ? 'Deceased' : data.memberType,
    membershipStatus: data.membershipStatus || 'Active',
    joinDate: data.joinDate || undefined,
    baptized: isDeceased ? undefined : data.baptized,
    baptizedDate: !isDeceased && data.baptized === 'Yes' ? data.baptizedDate : null,
    relationshipToHead: isDeceased ? undefined : data.relationshipToHead,
    relationCustom: data.relationshipToHead === 'Other' ? data.relationCustom?.trim() : undefined,
    maritalStatus: isDeceased ? undefined : (data.maritalStatus || 'Single'),
    spouseId,
    spouseName: data.spouseName || undefined,
    weddingDate: data.maritalStatus === 'Married' && data.weddingDate ? data.weddingDate : undefined,
    occupationCategory: isDeceased 
      ? 'Deceased' 
      : (age !== null ? resolveOccupation(age, data.occupationCategory) : data.occupationCategory),
  };
  ...
  // Auto-link spouse logic (only for non-Deceased)
  if (autoLinkedSpouse) {
    const head = await Believer.findById(family.headId);
